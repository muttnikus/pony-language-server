...
actor StdinStdoutLanguageServer
  new create(in_stream: InputStream,
    ...
    in_stream.apply(StdinNotify(dispatcher, logger, out_stream), chunk_size)
...
class iso StdinNotify is InputNotify
  ...
  fun ref _handle_request(pr: ParseResult, allow_batch: Bool = true): ( Promise[(jsonrpc.Response | jsonrpc.BatchResponse)] | None ) =>
    ...
    | let req: jsonrpc.Request val =>
      _logger(Fine) and _logger.log(req.to_json())
  ...
  fun ref apply(data: Array[U8] iso) =>
    ...
    for request in _parser do
      ...
      | let res: Promise[(jsonrpc.Response | jsonrpc.BatchResponse)] =>
        // write response
        res.next[None](
          {(response) => ResponseWriter.write(response, _out) },
          // promise is rejected if no handler is found
          { () =>
            ResponseWriter.write(
              jsonrpc.Response.from_error(None, jsonrpc.Error.method_not_found()),
              _out)
          })
      | None =>
